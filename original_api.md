...
**Note:** All of the URLs in the examples below are for the BETYdb instance at
  ...
  "https://www.betydb.org" with the top-level URL for the desired site.
...
* The portion of the URL directly following {{book.BETYdb_URL}}/ determines the
  ...
  use {{book.BETYdb_URL}}/citations.json to get results from the citations table
  in JSON format and {{book.BETYdb_URL}}/cultivars.xml to get results from the
...
* As a special case, to query the SQL view `traits_and_yields_view`, use
  ...
  {{book.BETYdb_URL}}/search.json?search=Salix+caprea [^limiting_results] returns
...
* URLs can not contain spaces.  If the value you are querying against contains a
   ...
   {{book.BETYdb_URL}}/species.csv?scientificname=Panicum+virgatum.  We could
   not have written it as
   {{book.BETYdb_URL}}/species.csv?scientificname=Panicum&nbsp;virgatum and
...
* Other tables involved in the query must be related to the primary table in one
  ...
  to include the sites table.  Visit {{book.BETYdb_URL}}/schemas to view a list
...
1. Return all citations with their associated sites and yields:
    ...
    {{book.BETYdb_URL}}/citations.xml?year=1990&include[]=sites&include[]=yields
...
row to be included in the search result.  Recall that a word in the value of the
search string _matches_ a column value if it is textually included in it.
